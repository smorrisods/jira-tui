...
- `src/lib.rs` — library surface so integration tests can drive the real code; `src/main.rs` is a thin binary (CLI args, terminal lifecycle, async run loop) built on a `tokio` multi-thread runtime, with event handling in `src/keys.rs` and `$EDITOR` suspend/resume in `src/editor_launch.rs` (both binary-only modules). The run loop races a `crossterm::EventStream` against the animation tick via `tokio::select!`; the render/refresh/switch-view/detail-load/transition/edit/field-mapping call sites dispatch live Jira REST calls onto `tokio::task::spawn_blocking` via `src/app/async_ops.rs` rather than blocking the render thread — demo/cache-only sessions still resolve inline since there's no network round-trip worth a spinner for. The onboarding credential-verification flow's own `load_issues` call remains synchronous (it's a one-time handoff during setup, not a repeated hot path); everything downstream of it, including the field-mapping lookup it offers next, is async.
