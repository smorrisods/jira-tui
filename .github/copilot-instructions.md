...
- `src/lib.rs` — library surface so integration tests can drive the real code; `src/main.rs` is a thin binary (CLI args, terminal lifecycle, async run loop) built on a `tokio` multi-thread runtime, with event handling in `src/keys.rs` and `$EDITOR` suspend/resume in `src/editor_launch.rs` (both binary-only modules). The run loop races a `crossterm::EventStream` against the animation tick via `tokio::select!`; blocking Jira/cache/git calls are progressively moving onto `tokio::task::spawn_blocking` (tracked against issue #16) rather than the render thread.
