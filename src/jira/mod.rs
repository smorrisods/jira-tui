...
impl Config {
    ...
    pub fn load() -> Option<Config> {
        ...
        let base_url = std::env::var("JIRA_BASE_URL")
            ...
            .unwrap_or_else(|| "https://your-org.atlassian.net".to_string());
...
mod live {
    ...
    pub fn fetch_detail(cfg: &Config, key: &str) -> Result<IssueDetail> {
        let path = format!(
            "/rest/api/3/issue/{key}?fields=summary,status,issuetype,priority,assignee,reporter,labels,components,parent,issuelinks,description,customfield_XXXXX"
        );
        ...
        Ok(IssueDetail {
            ...
            acceptance_criteria: f.get("customfield_XXXXX").cloned(),
            transitions: Vec::new(),
        })
    }
